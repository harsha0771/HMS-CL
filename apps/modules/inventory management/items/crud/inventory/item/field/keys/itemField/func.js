...
let fs = require('fs')
fs.mkdir('./database/modules/inventory management/items/inventory/item/field/keys/itemField', { recursive: true }, () => {
    const db = new sqlite3.Database('./database/modules/inventory management/items/inventory/item/field/keys/itemField/itemField.db');

// Create the table with columns of different data types
db.serialize(() => {
    db.run(`  
    CREATE TABLE IF NOT EXISTS records (
      id INTEGER PRIMARY KEY, parent TEXT, index TEXT, name TEXT, type TEXT, value TEXT, createdAt TEXT, updatedAt TEXT
    )
  `);
});
...
let check_values = async (input_data) => {
    let model = JSON.parse(`{"parent":{"key":"parent","type":"string","required":false,"default":null},"index":{"key":"index","type":"string","required":false,"default":null},"name":{"key":"name","type":"string","required":true,"default":null},"type":{"key":"type","type":"string","required":false,"default":null},"value":{"key":"value","type":"string","required":false,"default":null},"createdAt":{"key":"createdAt","type":"string","required":false,"default":null},"updatedAt":{"key":"updatedAt","type":"string","required":false,"default":null}}`);
...
exports.create = async (data) => {
   ...
   // console.log(data)
    ...
    return new Promise((resolve, reject) => {
        const { parent, index, name, type, value, createdAt, updatedAt } = data;

        db.run(
            'INSERT INTO records (parent, index, name, type, value, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?)',
            data.parent, data.index, data.name, data.type, data.value, data.createdAt, data.updatedAt,
            function (err) {
                if (err) {
                    console.error('Error creating record:', err);
                    resolve({ completed: false, error: 'Internal Server Error' });
                } else {
                    resolve({ completed: true, data: { id: this.lastID, parent, index, name, type, value, createdAt, updatedAt } });
                }
            }
        );
    });
...
exports.update = (id, data) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        db.run(
            'UPDATE records SET parent = ?, index = ?, name = ?, type = ?, value = ?, createdAt = ?, updatedAt = ? WHERE id = ?',
            [data.parent, data.index, data.name, data.type, data.value, data.createdAt, data.updatedAt, id],
            (err) => {
                if (err) {
