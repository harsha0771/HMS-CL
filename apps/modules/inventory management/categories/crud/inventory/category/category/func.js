...
let fs = require('fs')
fs.mkdir('./database/modules/inventory management/categories/inventory//category', { recursive: true }, () => {
    const db = new sqlite3.Database('./database/modules/inventory management/categories/inventory//category/category.db');

// Create the table with columns of different data types
db.serialize(() => {
    db.run(`  
    CREATE TABLE IF NOT EXISTS records (
      id INTEGER PRIMARY KEY, name TEXT, category TEXT, createdAt TEXT, updatedAt TEXT
    )
  `);
});
...
let check_values = async (input_data) => {
    let model = JSON.parse(`{"name":{"key":"name","type":"string","required":true,"default":null},"category":{"key":"category","type":"string","required":false,"default":null},"createdAt":{"key":"createdAt","type":"string","required":false,"default":null},"updatedAt":{"key":"updatedAt","type":"string","required":false,"default":null}}`);
...
exports.create = async (data) => {
   ...
   // console.log(data)
    ...
    return new Promise((resolve, reject) => {
        const { name, category, createdAt, updatedAt } = data;

        db.run(
            'INSERT INTO records (name, category, createdAt, updatedAt) VALUES (?, ?, ?, ?)',
            data.name, data.category, data.createdAt, data.updatedAt,
            function (err) {
                if (err) {
                    console.error('Error creating record:', err);
                    resolve({ completed: false, error: 'Internal Server Error' });
                } else {
                    resolve({ completed: true, data: { id: this.lastID, name, category, createdAt, updatedAt } });
                }
            }
        );
    });
...
exports.update = (id, data) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        db.run(
            'UPDATE records SET name = ?, category = ?, createdAt = ?, updatedAt = ? WHERE id = ?',
            [data.name, data.category, data.createdAt, data.updatedAt, id],
            (err) => {
                if (err) {
