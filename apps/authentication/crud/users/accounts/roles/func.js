...
let fs = require('fs')
fs.mkdir('./database/authentication/users/accounts/roles', { recursive: true }, () => {
    const db = new sqlite3.Database('./database/authentication/users/accounts/roles/roles.db');

// Create the table with columns of different data types
db.serialize(() => {
    db.run(`  
    CREATE TABLE IF NOT EXISTS records (
      id INTEGER PRIMARY KEY, name TEXT, code TEXT, sideBar TEXT, createdAt TEXT, updatedAt TEXT
    )
  `);
});
...
let check_values = async (input_data) => {
    let model = JSON.parse(`{"name":{"key":"name","type":"string","required":false,"default":null},"code":{"key":"code","type":"string","required":false,"default":null},"sideBar":{"key":"sideBar","type":"string","required":false,"default":"home"},"createdAt":{"key":"createdAt","type":"string","required":false,"default":null},"updatedAt":{"key":"updatedAt","type":"string","required":false,"default":null}}`);
...
exports.create = async (data) => {
   ...
   // console.log(data)
    ...
    return new Promise((resolve, reject) => {
        const { name, code, sideBar, createdAt, updatedAt } = data;

        db.run(
            'INSERT INTO records (name, code, sideBar, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?)',
            data.name, data.code, data.sideBar, data.createdAt, data.updatedAt,
            function (err) {
                if (err) {
                    console.error('Error creating record:', err);
                    resolve({ completed: false, error: 'Internal Server Error' });
                } else {
                    resolve({ completed: true, data: { id: this.lastID, name, code, sideBar, createdAt, updatedAt } });
                }
            }
        );
    });
...
exports.update = (id, data) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        db.run(
            'UPDATE records SET name = ?, code = ?, sideBar = ?, createdAt = ?, updatedAt = ? WHERE id = ?',
            [data.name, data.code, data.sideBar, data.createdAt, data.updatedAt, id],
            (err) => {
                if (err) {
