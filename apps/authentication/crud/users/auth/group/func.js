...
let fs = require('fs')
fs.mkdir('./database/authentication/users/auth/group', { recursive: true }, () => {
    const db = new sqlite3.Database('./database/authentication/users/auth/group/group.db');

// Create the table with columns of different data types
db.serialize(() => {
    db.run(`  
    CREATE TABLE IF NOT EXISTS records (
      id INTEGER PRIMARY KEY, name TEXT, code TEXT, users TEXT, roles TEXT, groups TEXT, createdAt TEXT, updatedAt TEXT
    )
  `);
});
...
let check_values = async (input_data) => {
    let model = JSON.parse(`{"name":{"key":"name","type":"string","required":false,"default":null},"code":{"key":"code","type":"string","required":false,"default":null},"users":{"key":"users","type":"string","required":false,"default":null},"roles":{"key":"roles","type":"string","required":false,"default":null},"groups":{"key":"groups","type":"string","required":false,"default":null},"createdAt":{"key":"createdAt","type":"string","required":false,"default":null},"updatedAt":{"key":"updatedAt","type":"string","required":false,"default":null}}`);
...
exports.create = async (data) => {
   ...
   // console.log(data)
    ...
    return new Promise((resolve, reject) => {
        const { name, code, users, roles, groups, createdAt, updatedAt } = data;

        db.run(
            'INSERT INTO records (name, code, users, roles, groups, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?)',
            data.name, data.code, data.users, data.roles, data.groups, data.createdAt, data.updatedAt,
            function (err) {
                if (err) {
                    console.error('Error creating record:', err);
                    resolve({ completed: false, error: 'Internal Server Error' });
                } else {
                    resolve({ completed: true, data: { id: this.lastID, name, code, users, roles, groups, createdAt, updatedAt } });
                }
            }
        );
    });
...
exports.update = (id, data) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        db.run(
            'UPDATE records SET name = ?, code = ?, users = ?, roles = ?, groups = ?, createdAt = ?, updatedAt = ? WHERE id = ?',
            [data.name, data.code, data.users, data.roles, data.groups, data.createdAt, data.updatedAt, id],
            (err) => {
                if (err) {
